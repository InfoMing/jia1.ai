...
- 所有可切换的主题色必须使用 `--ji-theme-*` CSS 变量，禁止在组件中新增写死的主题色。
- 全站主题配置集中维护在 `src/business/config/themeConfig.js`，组件不得自行读写主题缓存。
...
- 公共布局只维护导航、页面出口、页脚和全站主题，不维护登录、支付、点赞或其他业务状态。
- 导航中的站内页面使用命名路由，外部资讯和项目入口必须使用安全的新窗口链接。
- 桌面端导航保持单行，空间不足时必须折叠为可展开菜单，不得产生横向溢出。
