  ...
  computed: {
    columns() {
      return [
        ...
        { prop: 'market', label: '市场', width: '80px' },
  ...
  methods: {
    loadData() {
      ...
      gpApi.list(params).then(res => {
        ...
        // UnifyResultAdvice 将 IPage 转为 CustomPage，字段为 list/total 而非 records/total
    ...
    handleFollow(item) { item.followed = !item.followed; this.$message.success(item.followed ? '已添加关注' : '已取消关注') },
